		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-16">
			<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
				<article class="group relative bg-zinc-900/30 border border-zinc-800 rounded-[40px] p-12 hover:border-primary/30 transition-all duration-700">
					...
					<h2 class="text-3xl md:text-4xl font-black mb-6 text-white group-hover:text-primary transition-colors leading-tight tracking-tight">
						<a href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
					</h2>

					<p class="text-lg text-zinc-500 font-medium leading-relaxed mb-10 line-clamp-2"><?php echo esc_html(get_the_excerpt()); ?></p>
