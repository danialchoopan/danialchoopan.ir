...
<main class="py-32 grid-pattern relative bg-white dark:bg-zinc-950">
	<div class="container mx-auto px-6 relative z-10">
		<header class="max-w-3xl mb-32">
			<h1 class="text-6xl md:text-8xl font-black mb-10 tracking-tighter text-zinc-900 dark:text-white uppercase"><?php esc_html_e( 'Logs', 'devportfolio' ); ?></h1>
			<p class="text-xl text-zinc-600 dark:text-zinc-500 font-medium leading-relaxed"><?php esc_html_e( 'Technical deep dives, architectural patterns, and system retrospectives.', 'devportfolio' ); ?></p>
		</header>
				...
				<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
					<article id="post-<?php the_ID(); ?>" <?php post_class( 'group' ); ?>>
						<div class="flex flex-col gap-10">
							<div class="max-w-3xl">
								<div class="flex flex-wrap items-center gap-6 text-[9px] font-black uppercase tracking-[0.2em] text-zinc-500 dark:text-zinc-600 mb-8">
									...
									<span class="w-1 h-1 bg-zinc-300 dark:bg-zinc-800 rounded-full"></span>
									<span class="flex items-center gap-2">
								...
								<h2 class="text-4xl font-black mb-6 text-zinc-900 dark:text-white group-hover:text-primary transition-colors tracking-tight leading-tight">
									<a href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
								</h2>

								<p class="text-lg text-zinc-600 dark:text-zinc-500 font-medium leading-relaxed mb-10 line-clamp-3">
									<?php echo esc_html( get_the_excerpt() ); ?>
								</p>

								<a href="<?php the_permalink(); ?>" class="inline-flex items-center gap-4 text-[10px] font-black uppercase tracking-widest text-zinc-900 dark:text-white group-hover:gap-6 transition-all">
